...
class Site
{
    ...
    /**
     * Secure the given host with TLS. On Butler (Valet on Docker), we add/remove CA, Cert and trusting those two certs on butler script
     *
     * @param  string  $url
    ...
    public function createCertificate($url)
    {
        ...
        $this->cli->run(sprintf(
            'openssl x509 -req -sha256 -days 1095 -CA "%s" -CAkey "%s" %s -in "%s" -out "%s" -extensions v3_req -extfile "%s"',
    ...
    {
        $this->cli->run(sprintf(
            'openssl req -new -key "%s" -out "%s" -subj "/C=MY/ST=Negeri Sembilan/O=RunCloud/localityName=Mantin/commonName=%s/organizationalUnitName=Engineering/emailAddress=%s%s/" -config "%s"',
            $keyPath, $csrPath, $url, $url, '@laravel.butler', $confPath
        ));
    }
